...
<nav class="py-4 fixed inset-x-0 top-0 z-50 bg-white shadow">
    <div x-data="{ menu: (window.innerWidth >= 768) ? true : false }" class="container mx-auto px-8 flex items-center justify-between relative">
        <!-- Logo Teks Estetik -->
        <a href="{{ route('index') }}" class="text-2xl font-bold text-gray-800 font-serif tracking-wide">
            Graha Indah
        </a>

        <!-- Tombol menu mobile -->
        <button x-on:click="menu = (window.innerWidth >= 768) ? true : !menu" class="md:hidden btn btn-icon">
