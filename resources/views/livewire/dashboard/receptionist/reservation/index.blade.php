<div class="space-y-8">
                ...
                <tr class="bg-white border-b">
                    ...
                    <td class="td flex space-x-2">
                        ...
                        @if ($reservation->payment_proof)
                            <a target="_blank" rel="noopener noreferrer" href="{{ asset('storage/' . $reservation->payment_proof) }}" class="btn btn-sm btn-info">Lihat Bukti TF</a>
                        @endif
                    </td>
                </tr>
            @empty
    ...
    <!-- Modal Success (Confirmed, Check In, Check Out) -->
    @foreach (['confirmed', 'checkin', 'checkout'] as $status)
    <div x-data="{ open: false }">
        <div x-show="open" @status:{{ $status }}.window="open = true" style="display: none" x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']" :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
            ...
            <div x-show="open" x-transition.duration.300ms x-on:click="open = false" class="relative min-h-screen flex items-center justify-center p-4">
                <div x-on:click.stop x-trap.noscroll.inert="open" class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4">
                    <div class="text-center space-y-4">
                        ...
                        <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">Successfully {{ ucfirst($status) }}</h2>
                    </div>
                    <div class="flex space-x-2 justify-center">
                        <button type="button" x-on:click="open = false" class="btn">Okay</button>
                    </div>
                </div>
            </div>
        </div>
