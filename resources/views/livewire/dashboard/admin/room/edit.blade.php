<div class="space-y-8">
    <h1 class="text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full">Edit Room</h1>
    ...
    <form class="space-y-4" wire:submit.prevent='update'>
        <div class="grid sm:grid-cols-2 gap-4">
            ...
            <!-- Jenis Room -->
            <div class="form-control">
                <label class="label" for="type">Jenis Room</label>
            ...
            <div class="form-control">
                ...
                <input wire:model='total_rooms' class="input" type="number" id="total_rooms">
                @error('total_rooms')
                    <span class="invalid">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-control">
                ...
                <input wire:model='price' class="input" type="number" id="price">
                @error('price')
                    <span class="invalid">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-control">
                ...
                <input wire:model='description' class="input" type="text" id="description">
                @error('description')
                    <span class="invalid">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-control sm:col-span-2">
                <label class="label" for="explanation">Explanation</label>
                <div wire:ignore>
                    <textarea wire:model='explanation' id="explanation"></textarea>
                </div>
                @error('explanation')
                    <span class="invalid">{{ $message }}</span>
                @enderror
                ...
                <div class="flex flex-wrap gap-4">
                    @foreach ($facilities as $facility)
                        <label for="{{ $facility->code }}" class="bg-gray-100 py-2 px-4 rounded-tr-xl rounded-bl-xl flex items-center gap-1 cursor-pointer">
                            <input wire:model="selectedFacilities.{{ $facility->code }}" value="{{ $facility->code }}" class="checkbox cursor-pointer" type="checkbox" id="{{ $facility->code }}">
            ...
            <div x-data="{ isUploading: false, progress: 0 }"
                ...
                <div class="form-control relative">
                    ...
                    <input wire:model='image' class="sr-only" type="file" id="image">
                    @error('image')
                        <span class="invalid">{{ $message }}</span>
                    @enderror
                </div>
        ...
        document.addEventListener('livewire:load', function () {
            ClassicEditor
                ...
                .then(editor => {
                    ...
                    window.addEventListener('room:edited', () => {
